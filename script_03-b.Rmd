...
```{r}
top_inc <- inner_join(sep_old, sep_new, by = "id") %>%
...
```{r}
top_dec <- inner_join(sep_old, sep_new, by = "id") %>%
  mutate(diff = u_grad.y - u_grad.x) %>%
  arrange(diff) %>%
  slice(1:5)
...
```{r}
sc_1c <- full_join(top_inc, top_dec,
                   by = c("title", "name", ))

sc_1c %>%
  ggplot(aes(name.x, diff)) +
    geom_col() +
    labs(title = "Biggest Decreases in Enrollment",
         xlab = "Course Name") +
    coord_flip() +
    theme_bw() +
  fct_reorder("diff")
